...
func TestSourceValuePathWalker(t *testing.T) {
	...
	var sourcePathWalker = &ValuePathWalker{}
	if node, err := sourcePathWalker.Walk(rootNode, path); err != nil {
...
func TestSourceValuePathWalkerWithIndex(t *testing.T) {
	rootNode := ds.NewNode[string, interface{}]()
	nodeRef := rootNode
...
func TestSourceValuePathWalkerWithIndexAndKey(t *testing.T) {
	rootNode := ds.NewNode[string, interface{}]()
	nodeRef := rootNode
	for _, step := range []string{"app", "env", "dev"} {
		nodeRef = nodeRef.AddChild(step)
...
func TestSourceValuePathWalkerWithIndexAndAttribute(t *testing.T) {
	rootNode := ds.NewNode[string, interface{}]()
	nodeRef := rootNode
	for _, step := range []string{"app", "env", "dev"} {
		nodeRef = nodeRef.AddChild(step)
...
func TestSourceValuePathWalkerReturnsErrorWithIndexOutOfRange(t *testing.T) {
	rootNode := ds.NewNode[string, interface{}]()
	nodeRef := rootNode
	for _, step := range []string{"app", "env", "dev"} {
		nodeRef = nodeRef.AddChild(step)
	...
	pathParser := pathparser.NewPathParser()
	var steps, _ = pathParser.Parse("$app.'env'.'dev'.'regions'.2")
	var path = path.NewPath(steps)

	var sourcePathWalker = &ValuePathWalker{}
	if _, err := sourcePathWalker.Walk(rootNode, path); err == nil {
		t.Fail()
	}
}

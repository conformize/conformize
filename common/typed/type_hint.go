...
func TypeHintOf(v reflect.Value) TypeHint {
	...
	return typeHintMappings[v.Kind()]
}
...
func isTuple(val reflect.Value) bool {
	...
	elemType := reflect.ValueOf(val.Index(startIdx).Interface()).Type()

	startIdx++
	if startIdx == endIdx {
		return elemType != reflect.TypeOf(val.Index(endIdx).Interface())
	}

	for endIdx > startIdx {
		for _, idx := range []int{startIdx, endIdx} {
			if reflect.TypeOf(val.Index(idx).Interface()) != elemType {
				return true
			}
		}

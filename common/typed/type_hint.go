...
func isTuple(val reflect.Value) bool {
	...
	sliceLen := val.Len()
	idx := 0
	elemType := val.Index(idx).Type()
	if elemType.Kind() != reflect.Interface {
		return false
	}

	elemType = reflect.ValueOf(val.Index(idx).Interface()).Type()
	idx++
	for idx < sliceLen {
		currentElemType := reflect.ValueOf(val.Index(idx).Interface()).Type()
		if currentElemType != elemType {
			return true
		}
		idx++
	}

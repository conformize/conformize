...
func TestValuePathEvaluator(t *testing.T) {
	...
	var valuePathEvaluator = &ValuePathEvaluator{}
	if node, err := valuePathEvaluator.Evaluate(rootNode, path); err != nil {
...
func TestValuePathEvaluatorWithIndexAndKey(t *testing.T) {
	...
	pathParser := pathparser.NewPathParser()
	var steps, _ = pathParser.Parse("$app.'env'.'dev'.'regions'.1.'zone'")
	var path = path.NewPath(steps)
...
func TestValuePathEvaluatorWithIndexAndAttribute(t *testing.T) {
	...
	pathParser := pathparser.NewPathParser()
	var steps, _ = pathParser.Parse("$app.'env'.'dev'.'regions'.2.'zone'.attributes.'meta'")
	var path = path.NewPath(steps)
...
func TestValuePathEvaluatorReturnsErrorWithIndexOutOfRange(t *testing.T) {
	...
	var valuePathEvaluator = &ValuePathEvaluator{}
	if _, err := valuePathEvaluator.Evaluate(rootNode, path); err == nil {
		t.Fail()
	}
}

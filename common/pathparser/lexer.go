...
func newLexer(input string) *lexer {
	l := &lexer{input: input, currentState: lexStatesTransitions[OBJECT_IDENTIFIER_START]}
	l.run()
	return l
}
...
func (l *lexer) run() {
	if l.currentState.state != OBJECT_IDENTIFIER_START || !l.accepts() {
		...
		l.skip()
		l.currentState = lexStatesTransitions[l.currentState.nextState(l)]
		for {
			if l.accepts() {
				l.scanBuf = append(l.scanBuf, l.input[l.pos])
				l.skip()
			} else {
				if l.currentState.state == PROPERTY {
					if !isKeyword(string(l.scanBuf)) {
						l.add(UNEXPECTED)
						break
					}
				}
				...
				l.add(l.currentState.state)
				nextState := lexStatesTransitions[l.currentState.nextState(l)]

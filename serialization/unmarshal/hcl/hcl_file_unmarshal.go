...
func (hclUnmarshal *HclFileUnmarshal) Unmarshal(source serialization.SourceDataReader) (*ds.Node[string, interface{}], error) {
...
func parseBody(body *hclsyntax.Body, parent *ds.Node[string, interface{}]) error {
...
func decodeValue(val cty.Value, parent *ds.Node[string, interface{}], attrName string) {
	...
	case val.Type().IsMapType() || val.Type().IsObjectType():
		var objNode *ds.Node[string, interface{}]
		if attrName != "" {
	...
	case val.Type().IsListType() || val.Type().IsTupleType() || val.Type().IsSetType():
		...
		for _, item := range val.AsValueSlice() {
			...
			default:
				child := ds.NewNode[string, interface{}]()

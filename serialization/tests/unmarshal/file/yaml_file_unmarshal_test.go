...
func testYamlFileUnmarshalling(filePath string) (*ds.Node[string, interface{}], error) {
	var fileSource, _ = serialization.NewFileSource(filePath)
	var jsonFileUnmarshal = yaml.NewYamlUnmarshal(fileSource)
	return jsonFileUnmarshal.Unmarshal()
}

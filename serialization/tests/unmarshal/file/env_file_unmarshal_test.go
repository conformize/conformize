...
func testDotEnvFileUnmarshalling(filePath string) (*ds.Node[string, interface{}], error) {
	var fileSource, _ = serialization.NewFileSource(filePath)
	var dotEnvFileUnmarshal = env.NewEnvFileUnmarshal(fileSource)
	return dotEnvFileUnmarshal.Unmarshal()
}

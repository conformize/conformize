...
func (numGtOrEqPrd *NumberIsGreaterThanOrEqualPredicate[T]) Test(value typed.Valuable, args *typed.TupleValue) (bool, error) {
	valid, validErr := numGtOrEqPrd.Validate(value, args, numGtOrEqPrd.Schema())
	if !valid {
...
func (numGtOrEqPrd *NumberIsGreaterThanOrEqualPredicate[T]) Arguments() int {
	return 1
}

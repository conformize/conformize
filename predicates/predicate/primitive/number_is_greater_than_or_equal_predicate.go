...
func (numGtOrEqPrd *NumberIsGreaterThanOrEqualPredicate[T]) Test(value typed.Valuable) (bool, error) {
	valid, validErr := numGtOrEqPrd.Validate(value, numGtOrEqPrd.Args, numGtOrEqPrd.Schema())
	if !valid {
	...
	var vo T
	numGtOrEqPrd.Args.Elements[0].As(&vo)
	return v >= vo, nil
}

func (numGtOrEqPrd *NumberIsGreaterThanOrEqualPredicate[T]) ArgumentsLength() int {
	return 1
}

func (numGtOrEqPrd *NumberIsGreaterThanOrEqualPredicate[T]) Arguments(args *typed.TupleValue) {

...
func (numLessOrEqPrd *NumberIsLessThanOrEqualPredicate[T]) Test(value typed.Valuable) (bool, error) {
	...
	if numLessOrEqPrd.Args.Type().Hint() != typed.Number {
		return false, fmt.Errorf("expected a number argument, got %s", numLessOrEqPrd.Args.Type().Name())
	}
	...
	var vo T
	numLessOrEqPrd.Args.As(&vo)
	return vo >= v, nil
}

func (numLessOrEqPrd *NumberIsLessThanOrEqualPredicate[T]) ArgumentsCount() int {
	return 1
}

func (numLessOrEqPrd *NumberIsLessThanOrEqualPredicate[T]) Arguments(args typed.Valuable) {

...
func (numWithinRangePrd *NumberIsWithinRangePredicate[T]) Test(value typed.Valuable) (bool, error) {
	valid, validErr := numWithinRangePrd.Validate(value, numWithinRangePrd.Args, numWithinRangePrd.Schema())
	if !valid {
	...
	argIdx := 0
	var vLow T
	numWithinRangePrd.Args.Elements[argIdx].As(&vLow)

	argIdx++
	var vHigh T
	numWithinRangePrd.Args.Elements[argIdx].As(&vHigh)
	return v >= vLow && v <= vHigh, nil
}

func (numWithinRangePrd *NumberIsWithinRangePredicate[T]) ArgumentsLength() int {
	return 2
}

func (numWithinRangePrd *NumberIsWithinRangePredicate[T]) Arguments(args *typed.TupleValue) {

...
func (numEqPrd *NumberIsEqualPredicate[T]) Test(value typed.Valuable, args *typed.TupleValue) (bool, error) {
	valid, validErr := numEqPrd.Validate(value, args, numEqPrd.Schema())
	if !valid {
...
func (numEqPrd *NumberIsEqualPredicate[T]) Arguments() int {
	return 1
}

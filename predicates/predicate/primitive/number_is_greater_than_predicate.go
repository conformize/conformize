...
func (numGtPrd *NumberIsGreaterThanPredicate[T]) Test(value typed.Valuable) (bool, error) {
	valid, validErr := numGtPrd.Validate(value, numGtPrd.Args, numGtPrd.Schema())
	if !valid {
	...
	var vo T
	numGtPrd.Args.Elements[0].As(&vo)
	return v > vo, nil
}

func (numGtPrd *NumberIsGreaterThanPredicate[T]) ArgumentsLength() int {
	return 1
}

func (numGtPrd *NumberIsGreaterThanPredicate[T]) Arguments(args *typed.TupleValue) {
	numGtPrd.Args = args
}

...
type StringHasLengthPredicate struct {
	predicate.PredicateArgumentsValidator
	PredicateBuilder predicates.PredicateBuilder
}

func (strLenPrd *StringHasLengthPredicate) Test(value typed.Valuable, args *typed.TupleValue) (bool, error) {
	valid, validErr := strLenPrd.Validate(value, args, strLenPrd.Schema())
	if !valid {
	...
	supportedConditions := strLenPrd.supportedConditions()
	if _, ok := supportedConditions[cond]; !ok {
		return false, fmt.Errorf("unknown condition %s", conditionStr)
	}

	lenVal, _ := typed.NewNumberValue(len(s))
	condPrd, _ := strLenPrd.PredicateBuilder.Build(cond)
...
func (strLenPrd *StringHasLengthPredicate) Arguments() int {
	return 2
}

func (strLenPrd *StringHasLengthPredicate) supportedConditions() map[condition.ConditionType]struct{} {
...
func (strLenPrd *StringHasLengthPredicate) Schema() schema.Schemable {
	return &schema.Schema{
		Description: "String length predicate",
		Version:     1,

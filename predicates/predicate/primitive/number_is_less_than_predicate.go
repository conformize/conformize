...
func (numLtPrd *NumberIsLessThanPredicate[T]) Test(value typed.Valuable) (bool, error) {
	valid, validErr := numLtPrd.Validate(value, numLtPrd.Args, numLtPrd.Schema())
	if !valid {
	...
	var vo T
	numLtPrd.Args.Elements[0].As(&vo)
	return vo > v, nil
}

func (numLtPrd *NumberIsLessThanPredicate[T]) ArgumentsLength() int {
	return 1
}

func (numLtPrd *NumberIsLessThanPredicate[T]) Arguments(args *typed.TupleValue) {
	numLtPrd.Args = args
}

...
func (listIsSubsetPrd *ListIsSubsetPredicate) Test(value typed.Valuable, args *typed.TupleValue) (bool, error) {
	valid, validErr := listIsSubsetPrd.Validate(value, args, listIsSubsetPrd.Schema())
	if !valid {
	...
	for idx, elem := range listVal.Elements {
		...
		if elem.Type().Hint() == typed.List {
			return listIsSubsetPrd.Test(elem, &typed.TupleValue{

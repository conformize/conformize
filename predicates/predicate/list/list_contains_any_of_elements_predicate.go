...
func (listContainsAnyOfElemPrd *ListContainsAnyOfElementsPredicate) Test(value typed.Valuable) (bool, error) {
	valid, validErr := listContainsAnyOfElemPrd.Validate(value, listContainsAnyOfElemPrd.Args, listContainsAnyOfElemPrd.Schema())
	if !valid {
		return valid, validErr
	}

	listVal := value.(*typed.ListValue)
	elementsListVal := listContainsAnyOfElemPrd.Args.Elements[0].(*typed.ListValue)
...
func (listContainsAnyPrd *ListContainsAnyOfElementsPredicate) ArgumentsLength() int {
	return 1
}

func (listContainsAnyPrd *ListContainsAnyOfElementsPredicate) Arguments(args *typed.TupleValue) {

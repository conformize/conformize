...
func (listContainsAllPrd *ListContainsAllElementsPredicate) Test(value typed.Valuable) (bool, error) {
	valid, validErr := listContainsAllPrd.Validate(value, listContainsAllPrd.Args, listContainsAllPrd.Schema())
	if !valid {
		return valid, validErr
	}

	listVal := value.(*typed.ListValue)
	elementsListVal := listContainsAllPrd.Args.Elements[0].(*typed.ListValue)
...
func (listContainsAllPrd *ListContainsAllElementsPredicate) ArgumentsLength() int {
	return 1
}

func (listContainsAllPrd *ListContainsAllElementsPredicate) Arguments(args *typed.TupleValue) {
	listContainsAllPrd.Args = args
}

...
func (listElemMatchConditionPrd *ListElementsMatchConditionPredicate) Test(value typed.Valuable, args *typed.TupleValue) (bool, error) {
	valid, validErr := listElemMatchConditionPrd.Validate(value, args, listElemMatchConditionPrd.Schema())
	if !valid {
	...
	return listElemMatchConditionPrd.allMatch(listVal, cond, condArgs)
}

func (listElemMatchConditionPrd *ListElementsMatchConditionPredicate) allMatch(listVal *typed.ListValue, condition condition.ConditionType, args *typed.TupleValue) (bool, error) {
	for startIdx, endIdx := 0, len(listVal.Elements)-1; endIdx >= startIdx; startIdx, endIdx = startIdx+1, endIdx-1 {
		for _, idx := range []int{startIdx, endIdx} {
			...
			if listElemMatchConditionPrd.conditionPredicate == nil {
				prd, err := listElemMatchConditionPrd.PredicateBuilder.Build(condition)
				if err != nil {
					return false, err
				}
				listElemMatchConditionPrd.conditionPredicate = prd
			}

			ok, err := listElemMatchConditionPrd.conditionPredicate.Test(element, args)
			if !ok || err != nil {
				return false, err
			}
			if startIdx == endIdx {
...
func (listElemMatchConditionPrd *ListElementsMatchConditionPredicate) Schema() schema.Schemable {
	return &schema.Schema{
		Description: "All elements in a list meet condition predicate",
		Version:     1,

...
func (listEqPrd *ListIsEqualPredicate) Test(value typed.Valuable) (bool, error) {
	valid, validErr := listEqPrd.Validate(value, listEqPrd.Args, listEqPrd.Schema())
	if !valid {
		return valid, validErr
	}

	listVal := value.(*typed.ListValue)
	oListVal := listEqPrd.Args.Elements[0].(*typed.ListValue)
...
func (listEqPrd *ListIsEqualPredicate) ArgumentsLength() int {
	return 1
}

func (listEqPrd *ListIsEqualPredicate) Arguments(args *typed.TupleValue) {
	listEqPrd.Args = args
}

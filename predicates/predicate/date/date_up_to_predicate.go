...
func (dateUpToPredicate *DateUpToPredicate) Test(value typed.Valuable) (bool, error) {
	valid, validErr := dateUpToPredicate.Validate(value, dateUpToPredicate.Args, dateUpToPredicate.Schema())
	if !valid {
	...
	var oDateVal string
	oDateArg := dateUpToPredicate.Args.Elements[0]
...
func (dateUpTo *DateUpToPredicate) ArgumentsLength() int {
	return 1
}

func (dateUpTo *DateUpToPredicate) Arguments(args *typed.TupleValue) {
	dateUpTo.Args = args
}

...
func (dateIsPrd *DateIsPredicate) Test(value typed.Valuable) (bool, error) {
	valid, validErr := dateIsPrd.Validate(value, dateIsPrd.Args, dateIsPrd.Schema())
	if !valid {
	...
	oDateArg := dateIsPrd.Args.Elements[0]
	var oDateVal string
...
func (dateIsPrd *DateIsPredicate) ArgumentsLength() int {
	return 1
}

func (dateIsPrd *DateIsPredicate) Arguments(args *typed.TupleValue) {
	dateIsPrd.Args = args
}

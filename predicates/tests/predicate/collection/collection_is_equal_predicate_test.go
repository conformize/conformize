...
func TestCollectionIsEqualPredicate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			isEqPrd.Arguments(tt.args)
			got, err := isEqPrd.Test(tt.value)
...
func TestCollectionIsEqualPredicate_Arguments(t *testing.T) {
	tests := []struct {
		name string
		want int
	}{
	...
	isEqPrd := &collection.IsEqualPredicate{}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := isEqPrd.ArgumentsCount(); got != tt.want {
				t.Errorf("collection.IsEqualPredicate.Arguments() = %v, want %v", got, tt.want)
			}
		})
	}
}

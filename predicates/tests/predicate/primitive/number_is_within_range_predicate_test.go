...
func TestNumberIsWithinRangePredicate_Arguments(t *testing.T) {
	tests := []struct {
		name string
		want int
	}{
		{
			name: "returns 2 as expected number of arguments",
			want: 2,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := numWithinRangePrd.Arguments(); got != tt.want {

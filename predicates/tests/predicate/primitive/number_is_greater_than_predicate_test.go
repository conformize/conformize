...
func TestNumberIsGreaterThanPredicate(t *testing.T) {
	type args struct {
	...
	numGtPrd := &primitive.NumberIsGreaterThanPredicate[float64]{}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := numGtPrd.Test(tt.args.value, tt.args.args)
...
func TestNumberIsGreaterThanPredicate_Arguments(t *testing.T) {
	tests := []struct {
		name string
		want int
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := numGtPrd.Arguments(); got != tt.want {

...
func TestNumberIsGreaterThanPredicate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			numGtPrd.Arguments(tt.args.args)
			got, err := numGtPrd.Test(tt.args.value)
			if (err != nil) != tt.wantErr {
				t.Errorf("NumberIsGreaterThanPredicate.Test() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("NumberIsGreaterThanPredicate.Test() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func TestNumberIsGreaterThanPredicate_Arguments(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := numGtPrd.ArgumentsLength(); got != tt.want {
				t.Errorf("NumberIsGreaterThanPredicate.Arguments() = %v, want %v", got, tt.want)
			}
		})
	}
}

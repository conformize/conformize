...
func TestStringMatchesExpressionPredicate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				args: &typed.TupleValue{
					Elements:      []typed.Valuable{tests.PrimVal("^(\\d{1,3}\\.){3}\\d{1,3}$", &typed.StringTyped{})},
		...
		},
		{
			...
			args: args{
				...
				args: &typed.TupleValue{
					Elements:      []typed.Valuable{tests.PrimVal("[unclosed", &typed.StringTyped{})},
					ElementsTypes: []typed.Typeable{&typed.StringTyped{}},
				},
			},
			want:    false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			strMatchExprPrd.Arguments(tt.args.args)
			got, err := strMatchExprPrd.Test(tt.args.value)
...
func TestStringMatchesExpressionPredicate_Arguments(t *testing.T) {
	tests := []struct {
		name string
		want int
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := strMatchExprPrd.ArgumentsLength(); got != tt.want {
				t.Errorf("StringMatchesExpressionPredicate.Arguments() = %v, want %v", got, tt.want)
			}
		})
	}
}

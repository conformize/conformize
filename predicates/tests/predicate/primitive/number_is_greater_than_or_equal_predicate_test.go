...
func TestNumberIsGreaterThanOrEqualPredicate(t *testing.T) {
	type args struct {
	...
	tests := []struct {
		...
		{
			name: "returns true when value is greater than argument",
			args: args{
				value: tests.PrimVal(42, &typed.NumberTyped{}),
				args: &typed.TupleValue{
		...
		{
			name: "returns false when value is less than argument",
			args: args{
	...
	numGtOrEqPrd := &primitive.NumberIsGreaterThanOrEqualPredicate[float64]{}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			numGtOrEqPrd.Arguments(tt.args.args)
			got, err := numGtOrEqPrd.Test(tt.args.value)
			if (err != nil) != tt.wantErr {
				t.Errorf("NumberIsGreaterThanOrEqualPredicate.Test() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("NumberIsGreaterThanOrEqualPredicate.Test() = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestNumberIsGreaterThanOrEqualPredicate_Arguments(t *testing.T) {
	tests := []struct {
		name string
		want int
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := numGtOrEqPrd.ArgumentsLength(); got != tt.want {
				t.Errorf("NumberIsGreaterThanOrEqualPredicate.Arguments() = %v, want %v", got, tt.want)
			}
		})
	}
}

...
func TestNumberIsEqualPredicate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			numEqPrd.Arguments(tt.args.args)
			got, err := numEqPrd.Test(tt.args.value)
			if (err != nil) != tt.wantErr {
				t.Errorf("NumberIsEqualPredicate.Test() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("NumberIsEqualPredicate.Test() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func TestNumberIsEqualPredicate_Arguments(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := numEqPrd.ArgumentsLength(); got != tt.want {
				t.Errorf("NumberIsEqualPredicate.Arguments() = %v, want %v", got, tt.want)
			}
		})
	}
}

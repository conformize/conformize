...
func TestNumberIsEqualPredicate(t *testing.T) {
	type args struct {
	...
	tests := []struct {
		...
		{
			name: "returns false when value is greater",
			args: args{
				value: tests.PrimVal(42, &typed.NumberTyped{}),
				args: &typed.TupleValue{
	...
	numEqPrd := &primitive.NumberIsEqualPredicate[float64]{}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := numEqPrd.Test(tt.args.value, tt.args.args)
...
func TestNumberIsEqualPredicate_Arguments(t *testing.T) {
	tests := []struct {
		name string
		want int
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := numEqPrd.Arguments(); got != tt.want {

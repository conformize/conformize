...
func TestListHasNoDuplicateElementsPredicate(t *testing.T) {
	tests := []struct {
		name    string
	...
	listHasNoDupPrd := &list.ListHasNoDuplicateElementsPredicate{}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			listHasNoDupPrd.Arguments(&typed.TupleValue{})
			got, err := listHasNoDupPrd.Test(tt.value)
			if (err != nil) != tt.wantErr {
				t.Errorf("ListHasNoDuplicateElementsPredicate.Test() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("ListHasNoDuplicateElementsPredicate.Test() = %v, want %v", got, tt.want)
			}
		})
	}
}

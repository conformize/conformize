...
func TestListIsSubsetPredicate(t *testing.T) {
	tests := []struct {
		name    string
		...
		},
		{
			...
			args: &typed.TupleValue{
				Elements:      []typed.Valuable{typed.NewListValue([]typed.Valuable{}, &typed.NumberTyped{})},
				ElementsTypes: []typed.Typeable{&typed.ListTyped{ElementsType: &typed.NumberTyped{}}},
			},
			want:    false,
			wantErr: false,
		...
		{
			name: "returns false when repeating elements in a list have more occurences than in superset",
			value: typed.NewListValue(
				[]typed.Valuable{
					tests.PrimVal(1, &typed.NumberTyped{}),
					tests.PrimVal(1, &typed.NumberTyped{}),
		...
		{
			name: "returns true when repeating elements in a list have less occurences than in superset",
			value: typed.NewListValue(
				[]typed.Valuable{
					tests.PrimVal(1, &typed.NumberTyped{}),
					tests.PrimVal(1, &typed.NumberTyped{}),
	...
	listIsSubsetPrd := &list.ListIsSubsetPredicate{}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			listIsSubsetPrd.Arguments(tt.args)
			got, err := listIsSubsetPrd.Test(tt.value)
			if (err != nil) != tt.wantErr {
				t.Errorf("ListIsSubsetPredicate.Test() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("ListIsSubsetPredicate.Test() = %v, want %v", got, tt.want)
			}
		})
	}
}

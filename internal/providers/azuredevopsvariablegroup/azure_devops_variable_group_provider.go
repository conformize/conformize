...
func (azureDevOpsVarGrpPrvdr *AzureDevOpsVariableGroupProvider) Provide(queryRequest *sdk.ProviderDataRequest) (*ds.Node[string, interface{}], *diagnostics.Diagnostics) {
	...
	if varGrp.Variables != nil {
		for varName, varValue := range *varGrp.Variables {
			node, found := result.GetChild(varName)
			if !found {

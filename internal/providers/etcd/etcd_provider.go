...
type etcdClientConfig struct {
	Endpoints         []string
	ConnectionTimeout int16
	Authentication    *etcdClientAuth
}
...
func (etcdPrvdr *EtcdProvider) Provide(queryRequest *sdk.ProviderDataRequest) (*ds.Node[string, interface{}], *diagnostics.Diagnostics) {
	...
	for _, kv := range kvs {
		...
		if node, found := result.GetChild(string(key)); found {
			node.Value = value
		} else {

...
func (googleSecretMngrPvdr *GoogleSecretManagerProvider) Provide(req *sdk.ProviderDataRequest) (*ds.Node[string, interface{}], *diagnostics.Diagnostics) {
...
func getSecretValues(client *secretmanager.Client, secrets []*secretmanagerpb.Secret) (*ds.Node[string, interface{}], *diagnostics.Diagnostics) {
	...
	for _, secret := range secrets {
		...
		node, found := result.GetChild(secretName)
		if !found {
...
func queryByFilter(client *secretmanager.Client, project string, filter string, pageSize int32) (*ds.Node[string, interface{}], *diagnostics.Diagnostics) {
	diags := diagnostics.NewDiagnostics()

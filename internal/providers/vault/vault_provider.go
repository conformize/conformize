...
type vaultGithubAuthConfig struct {
	Path  string
	Token string
}
...
func (vaultPrvdr *VaultProvider) Provide(queryRequest *sdk.ProviderDataRequest) (*ds.Node[string, interface{}], *diagnostics.Diagnostics) {
	...
	var result = ds.NewNode[string, interface{}]()
	go func() {
		done := false
		for !done {
			select {
					...
					if metadata, ok := val.Data["metadata"]; ok {
						if metadataMap, ok := metadata.(map[string]interface{}); ok {
							if customMetadata, ok := metadataMap["custom_metadata"]; ok {
								if meta, ok := customMetadata.(map[string]interface{}); ok {

...
func (azureKVPrvdr *AzureKeyVaultProvider) Provide(queryRequest *sdk.ProviderDataRequest) (*ds.Node[string, interface{}], *diagnostics.Diagnostics) {
	...
	if len(queryOptions.SecretNames) > 0 {
		...
		for _, secret := range secrets {
			node, found := result.GetChild(secret.ID.Name())
			if !found {
...
func addAttributes(node *ds.Node[string, interface{}], attrs *azsecrets.SecretAttributes) {
	if attrs != nil {

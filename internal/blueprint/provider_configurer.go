...
func (prvdrCnfgr *providerConfigurer) Configure(provider api.ConfigurationProvider, sourceConfig elements.ConfigurationSource) error {
	configSchema := provider.ConfigurationSchema()
	prvdrConfReq := api.NewConfigurationRequest(configSchema)

	var providerConf map[string]interface{}
	if sourceConfig.ConfigFile == nil {
		providerConf = sourceConfig.Config
	} else if configFilePath, err := functions.InterpolateEnvVars(*sourceConfig.ConfigFile); err != nil {
		return err
	} else {
		if absConfigFilePath, err := util.FilePath(configFilePath); err != nil {
			return err
		} else {
			fileSrc, err := serialization.NewFileSource(absConfigFilePath)
			if err != nil {
				return err
			} else if fileContent, err := fileSrc.Read(); err != nil {
				return err
			} else if err := yaml.Unmarshal(fileContent, &providerConf); err != nil {
				return err
			}
		}
	}
	...
	if err := prvdrConfReq.Set(providerConf); err != nil {
		return err
	}
	return provider.Configure(prvdrConfReq)
}

...
func ruleNotSatisfiedErrorMessage(ruleMeta *elements.RuleMeta, ruleIdx int) string {
	...
	if len(ruleMeta.Name) > 0 {
		ruleHeader = fmt.Sprintf("Rule '%s':", ruleMeta.Name)
	} else {
		ruleHeader = fmt.Sprintf("Rule %d:", ruleIdx+1)
	}

	bldr.WriteString(format.Formatter().
	...
	writeField := func(label, value string) {
		...
		bldr.WriteString(bulletIndent + format.Formatter().

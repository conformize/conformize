...
func (blprntExecCtx blueprintExecutionContext) WithDiagnostics(diags diagnostics.Diagnosable) *blueprintExecutionContext {
...
func (blprntExecCtx blueprintExecutionContext) executeRule(rIdx int, r *elements.Rule) (bool, *elements.RuleMeta, error) {
	...
	if r.Arguments != nil {
		...
		case *elements.PathValue:
			valuePathSteps, err := pathParser.Parse(arg.Path)
			valNode, err := blprntExecCtx.valRefStore.GetAtPath(path.NewPath(valuePathSteps))
			if err != nil {
				blprntExecCtx.diags.
					Append(diagnostics.Builder().Error().
						Summary(fmt.Sprintf("Rule %d - Couldn't resolve value path %s for argument", ruleIdx, arg.Path)).
						Details(err.Error()).
						Build(),
					)
				return false, nil, err
			}
			argMeta.Path = arg.Path
			argVal, err = functions.ParseRawValue(valNode.Value)
			if err != nil {
				blprntExecCtx.diags.
					Append(diagnostics.Builder().Error().
						Details(fmt.Sprintf("\nRule %d - couldn't parse argument value at path %s, reason:\n%s", ruleIdx, arg.Path, err.Error())).
						Build(),
					)
				return false, nil, err
			}
...
type sourceJob struct {
	alias  string
	source *elements.ConfigurationSource
}
...
func (blprntExecCtx *blueprintExecutionContext) Execute() {
	...
	refResolver := NewReferencesResolver(blprntExecCtx.valRefStore)

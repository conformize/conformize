...
func (blprntExecCtx *blueprintExecutionContext) Execute() {
	...
	for idx, rule := range blprntExecCtx.blueprint.Ruleset {
		...
		if !ok {
			blprntExecCtx.diags.Append(diagnostics.Builder().
				Error().
				Details(blprntExecCtx.ruleViolationErrorMessage(ruleMeta, idx)).
				Build(),
			)
			ruleViolationsCount++
		}
...
func (blprntExecCtx *blueprintExecutionContext) ruleViolationErrorMessage(ruleMeta *elements.RuleMeta, ruleIdx int) string {
	blprntExecCtx.msgBldr.Reset()
	blprntExecCtx.msgBldr.Grow(256)
	...
	blprntExecCtx.msgBldr.WriteString(format.Formatter().
		Detail(format.Failure).
		Color(colors.Red).
		Bold().
		Format(ruleHeader))
	...
	writeLine := func(label, value string) {
		...
		blprntExecCtx.msgBldr.WriteString(
			bulletIndent +
				format.Formatter().
	...
	return blprntExecCtx.msgBldr.String()
}

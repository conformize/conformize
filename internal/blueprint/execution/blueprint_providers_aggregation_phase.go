...
func (phase *BlueprintProvidersAggregationPhase) Execute(blprntExecCtx *BlueprintExecutionContext) {
	...
	aggregatePrvdrs := make([]sdk.ConfigurationProvider, 0)
	for _, providerAlias := range depsOrdered {
		provider, exists := blprntExecCtx.providersRegistry.Get(providerAlias)
		if !exists {
			diags.Append(
				diagnostics.Builder().Error().
		...
		if _, isAggregate := provider.(*aggregate.AggregateProvider); isAggregate {
			aggregatePrvdrs = append(aggregatePrvdrs, provider)
		}
	}
	...
	for _, aggregatePrvdr := range aggregatePrvdrs {
		_, prvdDiags := aggregatePrvdr.Provide(sdk.NewProviderDataRequest(aggregatePrvdr.ProvisionDataRequestSchema()))

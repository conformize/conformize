...
func (blprntExecCtxBuilder *builder) withSource(alias string, sourceConfig elements.ConfigurationSource) {
	...
	if err == nil {
		...
		sourceDiags.Append(diagnostics.Builder().
			Error().
			Details(
				fmt.Sprintf("\nCouldn't read from source '%s' with '%s' provider, reason:\n",
...
func (blprntExecCtxBuilder *builder) withRule(rIdx int, r elements.Rule) {
	...
	for argIdx, arg := range r.Arguments {
			...
			if err != nil {
				blprntExecCtxBuilder.diags.
					Append(diagnostics.Builder().
						Error().
						Summary(
							fmt.Sprintf("Rule %d - Couldn't parse value path %s for argument %d",
								ruleIdx, argVal.Path, argIdx+1),
						).
						Details(err.Error()).
						Build(),
					)
		...
		if argVal, err := functions.ParseRawValue(argMeta.Value); err == nil {
	...
	blprntExecCtxBuilder.ruleSet[rIdx] = ruleBldr.Build()
}

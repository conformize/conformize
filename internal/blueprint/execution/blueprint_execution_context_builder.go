	...
	case *elements.PathValue:
		...
		if err != nil {
			blprntExecCtxBuilder.diags.
				Append(diagnostics.Builder().
					Error().
					Summary(fmt.Sprintf("Rule %d - Couldn't parse value path argument: %s", ruleIdx, argVal.Path)).
					Details(err.Error()).
					Build(),
				)
			return
		...
		if err != nil {
			blprntExecCtxBuilder.diags.
				Append(diagnostics.Builder().
					Error().
					Summary(fmt.Sprintf("Rule %d - Couldn't resolve value path %s for argument", ruleIdx, argVal.Path)).
					Details(err.Error()).
					Build(),
				)
			return
...
func (blprntExecCtxBuilder *builder) Build() *BlueprintExecutionContext {
	...
	blprntExecCtxBuilder.diags.Append(diagnostics.Builder().
		Info().
		Summary(format.Formatter().Detail(format.Box).Dimmed().Color(colors.Blue).Format(fmt.Sprintf("Reading %d sources...\n", sourcesCount))).
		Build(),
	)

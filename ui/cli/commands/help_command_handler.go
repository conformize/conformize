...
const commandNameColumnWidth = 15
...
func (h *HelpCommandHandler) Handle(c Commandable, args []string, diags diagnostics.Diagnosable) {
	...
	if len(args) == 0 {
		...
		helpBldr.WriteString(fmt.Sprintf(resources.GeneralHelp(), availableCmdsBldr.String()))
	} else {
		...
		for _, arg := range args[1:] {
			cmdNode, _ = cmdNode.GetChild(arg)
			cmd = cmdNode.Value
	...
	streams.Instance().Output().Writef("%s\n", helpBldr.String())
}

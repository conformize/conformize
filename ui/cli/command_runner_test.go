...
func TestNewCommandRunner(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: &commandRunner{
				cmdRegistry: commandregistry.Instance(),
			},
		},
	}
...
func Test_commandRunner_Run(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (tt.wantErr && !tt.args.diags.HasErrors()) || (!tt.wantErr && res != 0) {
				t.Errorf("CommandRunner.Run() error = %v, wantErr %v", tt.args.diags.Errors().Print(), tt.wantErr)
				return
			}
		})
	}

...
func (h *HelpCommandHandler) Handle(c CommandEntry, args []string, diags diagnostics.Diagnosable) {
	...
	if cmd == nil {
		streams.Error().Writeln(fmt.Sprintf("Unrecognized command %s", strings.Join(args, " ")))
		return
	}
